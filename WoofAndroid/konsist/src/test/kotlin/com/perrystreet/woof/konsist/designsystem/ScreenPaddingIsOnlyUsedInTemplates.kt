...
class ScreenPaddingIsOnlyUsedInTemplates : BehaviorSpec() {
    init {
        Given("A production file that applies a screen padding role") {
            val files = KonsistUtils.productionCode.files
    ...
    private companion object {
        ...
        private val Message = LintRuleMessage(
            ...
            badExample = """
                ...
                fun OrgFooList() {
                    Column(
                        modifier = Modifier.padding(horizontal = PaddingRoles.Screen.Horizontal.dp),
                    ) {
                        FooContent()
                    }
                }
                ...
                fun TemplateScrollableContent(content: LazyListScope.() -> Unit) {
                    LazyColumn(
                        contentPadding = PaddingValues(
                            start = PaddingRoles.Screen.Horizontal.dp,
                            end = PaddingRoles.Screen.Horizontal.dp,
                        ),
                        content = content,
                    )
                }
