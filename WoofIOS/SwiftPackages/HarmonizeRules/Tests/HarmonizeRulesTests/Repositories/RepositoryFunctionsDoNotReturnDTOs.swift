    ...
    override class func spec() {
        Given("A public member of a Repository") {
            ...
            Then("Its function return types never mention a DTO") {
                functions.assertFalse(rule: rule) { $0.returnClause?.description.contains("DTO") == true }
            }

            Then("Its property types never mention a DTO") {
                variables.assertFalse(rule: rule) { $0.typeAnnotation?.description.contains("DTO") == true }
            }
        }
    }

    private static let rule = Rule(
        description: "Repositories never expose DTOs.",
        rationale: "The repository is the boundary where transport data becomes domain data. Leaking a DTO pushes mapping into every caller.",
        fixHint: "Map with a DTOToDomainMapper inside the repository and return the domain model.",
